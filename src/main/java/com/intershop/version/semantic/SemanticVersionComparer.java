...
public class SemanticVersionComparer
{
     ...
     */
    public SemanticVersionComparer(SemanticVersionResolver resolver, Comparator<SemanticVersion> comparator)
    {
     ...
     */
    public String getNewestVersion(UpdateStrategy allowedChanges, Collection<String> versions, String current)
    {
        return getNewestVersion(allowedChanges, versions.stream().map(this::valueOf).collect(Collectors.toList()),
                        valueOf(current)).getVersion();
    }

    private SemanticVersion getNewestVersion(UpdateStrategy allowedChanges, Collection<SemanticVersion> versions,
     ...
     * @return the newest patch version or if no version could be determined the current
     */
    private SemanticVersion getNewestPatchVersion(Collection<SemanticVersion> versions, SemanticVersion current)
    {
     ...
     */
    private SemanticVersion getNewestMinorVersion(Collection<SemanticVersion> versions, SemanticVersion current)
    {
        Optional<SemanticVersion> firstElement = versions.stream()
                                                         .filter(v -> v.getMajor() == current.getMajor())
                                                         .filter(SemanticVersion::isRecommendedForProduction)
     ...
     */
    private SemanticVersion getNewestMajorVersion(Collection<SemanticVersion> versions, SemanticVersion current)
    {
        Optional<SemanticVersion> firstElement = versions.stream()
                                                         .filter(SemanticVersion::isRecommendedForProduction)
                                                         .max(comparator);
     ...
     * @param current version
     * @param versions available version
     * @return the newest major version or if no version could be determined the current, also non production versions can be selected.
     */
    private SemanticVersion getNewestAvailableVersion(Collection<SemanticVersion> versions, SemanticVersion current)
    {
        return versions.stream().max(comparator).orElse(current);
    }

    /**
     * Verify that a new version can be reached with a given update strategy.
     * The function doesn't check that the new version is really newer.
     * The function doesn't check that the version is "production" ready.
     * 
